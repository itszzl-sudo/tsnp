...
pub fn find_ffi_functions_local(path: &str) -> Result<Vec<FFIFunction>> {
    use std::path::Path;
...
pub fn find_ffi_functions_tarball(repo_url: &str) -> Result<Vec<FFIFunction>> {
    ...
    let response = reqwest::blocking::Client::new()
        ...
        .send()
        .context("GitHub download error")?;
    
    if !response.status().is_success() {
        anyhow::bail!("GitHub download failed: {}", response.status());
    }
...
pub fn find_ffi_functions_search(repo_url: &str, token: &str) -> Result<Vec<FFIFunction>> {
    let (owner, repo) = parse_github_url(repo_url)?;
    ...
    let response = reqwest::blocking::Client::new()
        ...
        .send()
        .context("GitHub Search API error")?;
    
    if !response.status().is_success() {
        anyhow::bail!("GitHub Search API failed: {}", response.status());
    }
...
fn extract_rust_files(tarball: &[u8]) -> Result<Vec<(String, String)>> {
    ...
    for entry in archive.entries().context("Tar error")? {
        let mut entry = entry.context("Entry error")?;
        
        let path = entry.path().context("Path error")?;
